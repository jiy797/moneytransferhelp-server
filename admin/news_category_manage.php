...
if($cat_name==''){
	...
	header("Location: news_category_manage_frm.php?id=$id&start=$start");
	exit();	
}
...
if($id==''){
	$sql="select cat_name from yp_news_category where cat_name='$cat_name'";
	...
	if($res==$cat_name){
		...
		$sql="insert into yp_news_category(cat_name, status) values('$cat_name', 'Active')";
		executeQuery($sql);
	...
	$sql="select cat_name from yp_news_category where cat_name='$cat_name' and cat_id<>'$id'";
	$res=getSingleResult($sql);
	...
	if($res==$cat_name){
		...
		$sql="update yp_news_category set cat_name='$cat_name' where cat_id='$id'";
		executeQuery($sql);
