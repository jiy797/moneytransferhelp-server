...
if($rec_name==''||$rec_desc==''){
	...
	header("location: resource_main_manage_frm.php?id=$id");	
	exit();
}
...
if($id!=''){
	$sql="update yp_resource_main set rec_name='$rec_name', rec_desc='$rec_desc', rec_img='$image_real', meta_title='$meta_title', meta_keywords='$meta_keywords', meta_desc='$meta_desc' where rec_id='$id'";
	executeQuery($sql);
	$sess_msg="RECORD UPDATED SUCESSFULLY.";
	$_SESSION['sess_msg']=$sess_msg;
}else{
	$max_pos=getsingleresult("select max(rec_position) from yp_resource_main");
	$max_pos=$max_pos+1;

	$sql="insert into yp_resource_main(rec_name, rec_desc, rec_img, status, posttime, rec_position, meta_title, meta_keywords, meta_desc) values('$rec_name', '$rec_desc', '$image_real', 'Active', now(), '$max_pos', '$meta_title', '$meta_keywords', '$meta_desc')";
	executeQuery($sql);	
...
header("location: resource_main_list.php");	
exit();
?>