<?php
	...
	if($_SERVER['HTTP_HOST']=="localhost" || $_SERVER['HTTP_HOST']=="youngpetals" || $_SERVER['HTTP_HOST']=="young" || $_SERVER['HTTP_HOST']=="192.168.0.2" || $_SERVER['HTTP_HOST']=="192.168.0.1"){
			...
			$DB["host"]				= "localhost";
			$DB["dbName"]			= "moneytra_vw"; 
			$DB["user"]				= "moneytra_vw";
			$DB["pass"]				= "abCDefN11";

			$non_secure_path		= "http://www.moneytransferhelp.com/";
			$non_secure_adminpath	= "http://www.moneytransferhelp.com/admin/";
			$secure_server_path		= "http://www.moneytransferhelp.com/";
			$SITE_PATH				= 'http://www.moneytransferhelp.com/';
			$SSL_PATH				= 'http://www.moneytransferhelp.com/';
			$SITE_FS_PATH			= '/home/moneytra/public_html/';
			$local_mode				= false;
	...
	function getRandomCode(){
		...
		$random_str =$string{0}.$string{1}.$string{2}.$string{3}.$string{4}.$string{5};
		return $random_str;
	}
	...
	function make_date($date,$format='us',$seperator='-')
		{
			if(strlen($date)>=10)
			{
	...
	function checkInput($inputText){
		$outputText=mysql_real_escape_string($inputText);
		return $outputText;
	}
	...
	function breadCrump($id){
			if($id!=""){
				...
				foreach($breads as $slice => $id_catname ){
					...
					echo " &raquo; <a href='category.php?cid=$cid'> $catname</a>";
				}
			}
		}
	...
	function country_combo($selected='United States',$combo_name=''){
		...
		echo "<select name='"; if($combo_name==''){ echo 'country'; }else{ echo $combo_name; } echo "' class='textfield'>";
	...
	function getPageContents($id){
		$page_content=array();
		$sql="select * from yp_newpages where status='Active' and page_id='$id'";
		$result=executeQuery($sql);
		if($line=mysql_fetch_array($result)){
	...
	function display_file_names($selected=''){
		...
		$file_list= array('Home','Advertising','Info','Links','Contact','Login','Registration');
	...
	function showConvertedPrice ($price, $lang="_en" ){
		...
		if(isset($_SESSION['my_currency_id']) && !empty($_SESSION['my_currency_id'])){
			...
			$converted_price = $currency_sign.number_format($price*$conversion_value,2,".",",");
		}else{
	...
	function watermarkImage ($SourceFile, $WaterMarkText, $DestinationFile) {
		...
		$black		= imagecolorallocate($image_p, 252,26,112);
		$font		= 'arial.ttf';
		if($width>350){
			$font_size	= 14;
		}else{
		...
		$position_x = $width/2-2*strlen($WaterMarkText);
		$position_y = $height/2;
		imagettftext($image_p, $font_size, 0, $position_x, $position_y, $black, $font, $WaterMarkText);
...
//---------------------------------------------Thumb Class-------------
	...
	function getPermission($id,$user){
	$sql="select read_page, write_page, delete_page from yp_admin_permission where pages_id='$id' and admin_id='$user'";
	$rs=executeQuery($sql);
	if($ls=mysql_fetch_array($rs)){
		$permission_value[0]=$ls['read_page'];
		$permission_value[1]=$ls['write_page'];
		$permission_value[2]=$ls['delete_page'];
	}
	return $permission_value;
	}

	function getPages($page){
		$sql="select pages_id from yp_admin_pages where pages='".strtolower($page)."'";
