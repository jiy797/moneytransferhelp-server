...
function DisplayCategory($catid='',$sep='',$pcatid=0,$selected=''){
	$sql="select category_id, category_name from yp_category where parent_cat_id='$pcatid' and category_status='Active' order by category_posttime";
...
function DisplaySubCategory($subid,$sep,$pcid,$subselected=''){
	$sep.="&raquo;";
	$sql="select * from yp_category where parent_cat_id='$subid' and category_status='Active' order by category_posttime";
	$r1=executeQuery($sql);
...
function DisplayResCategory($catid='',$sep='',$pcatid=0,$selected=''){
	$sql="select category_id, category_name from yp_resource_category where parent_cat_id='$pcatid' and category_status='Active' order by category_posttime";
	$result=executeQuery($sql);
	//echo "<br>$sql<br>";
	//exit();
	while($line=mysql_fetch_array($result)){
...
function DisplaySubResCategory($subid,$sep,$pcid,$subselected=''){
	$sep.="&raquo;";
	$sql="select * from yp_resource_category where parent_cat_id='$subid' and category_status='Active' order by category_posttime";
	$r1=executeQuery($sql);
...
function DisplayCategoryLang($catid='',$sep='',$pcatid=0,$selected='', $lang=''){
	$sql="select category_id, category_name, category_name_fr from yp_category where parent_cat_id='$pcatid' and category_status='Active' order by category_posttime";
	$result=executeQuery($sql);
	//echo "<br>$sql<br>";
	//exit();
	while($line=mysql_fetch_array($result)){
			...
			foreach($arr_f as $fol_id => $fol_name){
				...
				while($line_products=mysql_fetch_array($res_products)){
					...
					if($remove_images=='Yes'){
						$image_real =  getSingleResult("select product_real from yp_product where product_id = '".$line_products['product_id']."' ");
						$image_big =  getSingleResult("select product_big from yp_product where product_id = '".$line_products['product_id']."' ");
						$image_thumb =  getSingleResult("select product_thumb from yp_product where product_id = '".$line_products['product_id']."' ");
						@unlink($SITE_FS_PATH."uploadedfiles/real/".$image_real);
						@unlink($SITE_FS_PATH."uploadedfiles/big/".$image_big);
						@unlink($SITE_FS_PATH."uploadedfiles/thumb/".$image_thumb);
					}
					//---------------------------Remove Images----------------------
					$sql="delete from yp_product where product_id = '".$line_products['product_id']."'";
					executeQuery($sql);
				}
