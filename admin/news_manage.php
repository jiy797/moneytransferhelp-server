...
if($cat_id=='0'||$rec_name==''||$rec_date==''||$rec_desc==''){
	...
	header("location: news_manage_frm.php?id=$id");	
	exit();
}
...
if($id!=''){
	$sql="update yp_news set cat_id='$cat_id', rec_name='$rec_name',  rec_date='$rec_date', rec_desc='$rec_desc', rec_pdf='$image_real', meta_title='$meta_title', meta_keywords='$meta_keywords', meta_desc='$meta_desc' where rec_id='$id'";
	executeQuery($sql);
	$sess_msg="RECORD UPDATED SUCESSFULLY.";
	$_SESSION['sess_msg']=$sess_msg;
}else{
	$max_pos=getsingleresult("select max(rec_position) from yp_news");
	$max_pos=$max_pos+1;

	$sql="insert into yp_news(cat_id, rec_name, rec_date, rec_desc, rec_pdf, status, posttime, rec_position, meta_title, meta_keywords, meta_desc) values('$cat_id', '$rec_name', '$rec_date', '$rec_desc', '$image_real', 'Active', now(), '$max_pos', '$meta_title', '$meta_keywords', '$meta_desc')";
	executeQuery($sql);	
