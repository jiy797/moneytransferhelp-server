...
if($rec_name==''||$rec_name==''){
	...
	header("Location: learning_comment_view.php?id=$id&rec_id=$rec_id");
	exit();
}

if($rec_id!=''){
	$sql="update yp_learning_comment set rec_name='$rec_name', rec_desc='$rec_desc' where rec_id='$rec_id'";
	executeQuery($sql);
