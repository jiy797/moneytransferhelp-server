...
if(count($arr_ids)>0){
	...
	if($_POST['delete']!=''){
		...
		$sql_gal="select * from yp_resource_main where rec_id in ('$str_ids') " ;
		...
		foreach($arr_ids as $aid){
			...
			$sql="select * from yp_resource_main where rec_id='$aid'";
		...
		$sql="delete from yp_resource_main where rec_id in ('$str_ids') " ;
		executeUpdate($sql);
	}
	else if($_POST['Active']!=''){
		$sql="update yp_resource_main set status= 'Active' where rec_id in ('$str_ids') " ;	
		executeUpdate($sql);
	}
	else if($_POST['Inactive']!=''){
		$sql="update yp_resource_main set status= 'Inactive' where rec_id in ('$str_ids') " ;	
		executeUpdate($sql);
	}else if($_POST['Feature']!=''){
		$sql="update yp_resource_main set feature_status='Yes' where rec_id in ('$str_ids') " ;	
		executeUpdate($sql);
	}
	else if($_POST['Unfeature']!=''){
		$sql="update yp_resource_main set feature_status='No' where rec_id in ('$str_ids') " ;	
		executeUpdate($sql);
	}
}
