...
if($b_page==''){
	...
	header("location: banner_right_manage_frm.php?id=$id");	
	exit();
}
...
if($id!=''){
	$sql="update yp_banner_right set b_type='$b_type', b_page='$b_page', b_img='$image_real', b_link='$b_link', b_code='$b_code' where b_id='$id'";
	executeQuery($sql);
	$sess_msg="RECORD UPDATED SUCESSFULLY.";
	$_SESSION['sess_msg']=$sess_msg;
}else{
	$sql="insert into yp_banner_right(b_type, b_page, b_img, b_link, b_code, status) values('$b_type', '$b_page', '$image_real', '$b_link', '$b_code', 'Active')";
	executeQuery($sql);	
...
header("location: banner_right_list.php");	
exit();
?>