...
if($rec_name==''||$rec_date==''||$rec_desc==''){
	$sess_msg="PLEASE FILL ALL THE REQUIRED FIELDS.";
	$_SESSION['sess_msg']=$sess_msg;			
	header("location: mtb_manage_frm.php?id=$id");	
	exit();
}
...
if($id!=''){
	$sql="update yp_mtb set cat_id='$cat_id', rec_name='$rec_name',  rec_date='$rec_date', rec_desc='$rec_desc', rec_pdf='$image_real', meta_title='$meta_title', meta_keywords='$meta_keywords', meta_desc='$meta_desc' where rec_id='$id'";
	executeQuery($sql);
	$sess_msg="RECORD UPDATED SUCESSFULLY.";
	$_SESSION['sess_msg']=$sess_msg;
}else{
	$max_pos=getsingleresult("select max(rec_position) from yp_mtb");
	$max_pos=$max_pos+1;

	$sql="insert into yp_mtb(cat_id, rec_name, rec_date, rec_desc, rec_pdf, status, posttime, rec_position, meta_title, meta_keywords, meta_desc) values('$cat_id', '$rec_name', '$rec_date', '$rec_desc', '$image_real', 'Active', now(), '$max_pos', '$meta_title', '$meta_keywords', '$meta_desc')";
	executeQuery($sql);	
