<?php
	...
	@extract($_SESSION);
		...
		if($_SERVER['HTTP_HOST']=="localhost" || $_SERVER['HTTP_HOST']=="youngpetals" || $_SERVER['HTTP_HOST']=="young" || $_SERVER['HTTP_HOST']=="192.168.0.2" || $_SERVER['HTTP_HOST']=="192.168.0.1"){
			...
			$non_secure_path		= "http://".$_SERVER['SERVER_NAME']."/precisionaerobatics/site/";
			$non_secure_adminpath	= "http://".$_SERVER['SERVER_NAME']."/precisionaerobatics/site/admin/";
			$secure_server_path		= "http://".$_SERVER['SERVER_NAME']."/precisionaerobatics/site/";
			$SITE_PATH				= "http://".$_SERVER['SERVER_NAME']."/precisionaerobatics/site/";
			$SSL_PATH				= "http://".$_SERVER['SERVER_NAME']."/precisionaerobatics/site/";
			$SITE_FS_PATH			= "d:/apache/htdocs/lookatme/site/";
			$local_mode				= true;
		}else{
			$DB["host"]				= "localhost";
			$DB["dbName"]			= "youngpet_precision"; 
			$DB["user"]				= "youngpet_clients";
			$DB["pass"]				= "clients";

			$non_secure_path		= "http://clients.youngpetals.net/precision/";
			$non_secure_adminpath	= "http://clients.youngpetals.net/precision/admin/";
			$secure_server_path		= "http://clients.youngpetals.net/precision/";
			$SITE_PATH				= 'http://clients.youngpetals.net/precision/';
			$SSL_PATH				= 'http://clients.youngpetals.net/precision/';
			$SITE_FS_PATH			= '/home/youngpet/public_html/clients/precision/';
			$local_mode				= false;
		}	
	...
	function getPageContents($id){
		...
		if($line=mysql_fetch_array($result)){
			...
			$page_content[2]=$line['page_meta_keywords'];
			$page_content[3]=$line['page_meta_desc'];
		}
		return $page_content;
	}
