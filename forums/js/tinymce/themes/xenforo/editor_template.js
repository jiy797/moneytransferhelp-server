			...
			button = controlManager.createColorSplitButton('xenforo_smilies', {
				...
				onselect: function(s) {
					editor.execCommand('mceInsertContent', false,
						'<img src="' + DOM.encode(s) + '" alt="" class="smilie" data-smilie="yes" />'
					);
				},
				smilies: smilies
	...
	tinymce.create('tinymce.plugins.XenForoElastic', {
		init: function(ed)
		{
			...
			eventResize = function()
			{
				...
				if (!$.browser.webkit) // webkit scrollHeight is the document height, not content height
				{
				...
				if (height != curHeight)
				{
					if (!$.browser.msie) // IE doesn't need this ?!?! (full size images cause problems with this)
					{
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.XenForoSmiliesButton:tinymce.ui.SplitButton', {
		...
		renderMenu : function() {
			...
			n = DOM.add(m, 'table', {'class' : 'mceSmiliesMenu'});
			tb = DOM.add(n, 'tbody');
			...
			each(s.smilies, function(smilie, smilieName)
			{
				...
				}, '<img src="' + DOM.encode(smilie[1]) + '" alt="" title="' + DOM.encode(smilie[0]) + '" />');
			});
			...
			Event.add(t.id + '_menu', 'click', function(e) {
				...
				if ($t.length)
				{
					t.settings.onselect($t.data('smilieUrl'));
					t.hideMenu();
				}
...
*/
		...
		_addStatusBar : function(tb, o) {
			...
			n = DOM.add(n, 'div', {id : ed.id + '_path_row'}, s.theme_xenforo_path ? ed.translate('xenforo.path') + ': ' : '&#160;');
