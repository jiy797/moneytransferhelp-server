...
!function($, window, document, _undefined)
{
	...
	XenForo.FilterList.prototype =
	{
		...
		handleLast: function()
		{
			var last = /(&|\/?)last=(.*)(&|$)/.exec(window.location.search),
				$last;

			if (last)
			{
				$last = $('#listItem_' + last[2]);
				if ($last.length)
				{
					$last.get(0).scrollIntoView(false);
					$last.addClass('last');
				}
			}
		},
		...
		showHideNoResults: function(show)
		{
			...
			if (show)
			{
				if (!$noRes.length)
				{
					$noRes = $('<li id="noResults" class="listNote" style="display:none">No items matched your filter.</li>');
