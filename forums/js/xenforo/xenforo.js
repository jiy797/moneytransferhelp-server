...
!function($, window, document, _undefined)
{
	...
	jQuery.fn.extend(
	{
		 ...
		 *
		 * @param integer|string Speed of fade-out
		 * @param function Callback function on completion
		 * @param integer|string Slide speed
		...
		xfFadeUp: function(fadeSpeed, callback, slideSpeed, easingMethod)
		{
			return this
				...
				.slideUp(
				{
					duration: slideSpeed || XenForo.speed.slow,
	...
	$.extend(XenForo,
	{
			...
			if ($('a.twitter-share-button').length)
			{
				try
				{
					var twitScript = document.createElement('script');
						twitScript.async = true;
						twitScript.src = 'http://platform.twitter.com/widgets.js';
		...
		createOverlay: function($trigger, templateHtml, extraOptions)
		{
			...
			noFixed = ($.browser.msie && $.browser.version <= 6) || XenForo._isWebkitMobile;
	...
	{
		$form.find('input[name="keywords"]').focus(function(focusEvent)
		{
				...
				XenForo.ajax(loadUrl, {}, function(ajaxData)
				{
					if (XenForo.hasTemplateHtml(ajaxData) || XenForo.hasTemplateHtml(ajaxData, 'message'))
					{
						new XenForo.ExtLoader(ajaxData, function(ajaxData)
						{
							...
							$(ajaxData.templateHtml || ajaxData.message).xfInsert('appendTo', $pane, 'xfFadeIn', 0);
						});
					}
				}, {type: 'GET'});
			}
	...
	XenForo.SpinBox.prototype =
	{
		__construct: function($input)
		{
			...
			// set up the buttons
			this.$wrapper.find('input:button')
				.focus($.context(this, 'eFocusButton'))
		...
		/**
		 * Handles the input being focused. Attaches the 'pseudofocus' class.
		 *
		 * @param Event e
		 */
		eFocusInput: function(e)
	...
	{
		var $input, $button, $list, setCheckedClasses;
	...
	XenForo.AutoValidator.prototype =
	{
		...
		ajaxSave: function(e)
		{
			...
			if (!eDataSend.isDefaultPrevented())
			{
	...
	XenForo.FieldAdder = function($button)
	{
		$button.click(function(e)
		{
			...
			if ($source.length && (!maxFields || ($source.length < maxFields)))
			{
				...
				{
					$clone.find('input').first().focus().select();
