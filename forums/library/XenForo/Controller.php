...
abstract class XenForo_Controller
{
	...
	*/
	 ...
	 */
	public function responseException(XenForo_ControllerResponse_Abstract $controllerResponse)
	{
	...
	*/
	...
	public function ipMatch($checkIps, array $ipList)
	{
		...
		/*foreach ($clientIps AS $ip)
		{
			...
			if (isset($bannedIps[$ipClassABlock]))
			{
				foreach ($bannedIps[$ipClassABlock] AS $range)
				{
					if ($long >= $range[0] && $long <= $range[1])
					{
	...
	protected function _getClientIps()
	{
		return array_unique(array(
			$this->_request->getClientIp(true),
