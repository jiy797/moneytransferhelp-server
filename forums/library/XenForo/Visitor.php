...
class XenForo_Visitor implements ArrayAccess
{
	...
	*/
	...
	public static function setup($userId, array $options = array())
	{
		...
		if ($object->_user['language_id'] && !empty($languages[$object->_user['language_id']]))
		{
			$language = $languages[$object->_user['language_id']];
		}
		else
		{
		...
		XenForo_Locale::setDefaultLanguage($language, $object->_user['timezone']);
	...
	{
		return XenForo_Model::create('XenForo_Model_User')->isMemberOfUserGroup($this->_user, $userGroupId, $includeSecondaryGroups);
	}
