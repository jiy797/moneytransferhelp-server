...
class XenForo_Template_Helper_Admin
{
	...
	protected static function _getTextInput($type, $name, $value, array $controlOptions, $id = '')
	{
		...
		if (!empty($controlOptions['rows']) && $type == 'text')
		{
			...
			return "<textarea name=\"{$name}\" rows=\"{$controlOptions['rows']}\" cols=\"{$cols}\"{$title}{$placeholder}{$autofocus}{$wrap}{$dir} class=\"textCtrl{$inputClass}\"{$dataAttributes} id=\"{$id}\">{$value}</textarea>";
		}
		else
		{
			return "<input type=\"{$type}\" name=\"{$name}\" value=\"{$value}\"{$size}{$maxlength}{$title}{$placeholder}{$autofocus}{$autocomplete}{$dir}{$step}{$min}{$max}{$results} class=\"textCtrl{$inputClass}\"{$dataAttributes} id=\"{$id}\" />";
		}
	}
	...
	protected static function _getSelectOptionHtml($value, $label, $selected, array $extra = array())
	{
		...
		return "<option value=\"$value\"$class$selectedHtml>$prefix$label</option>\n";
	}
	...
	protected static function _getCheckBoxOptionHtml($value, $label, $selected, array $extra = array(), array $options = array())
	{
		...
		if (!empty($extra['name']))
		{
			if ($extra['name'][0] == '[' && isset($options['defaultName']))
			{
		...
		return "<li><label for=\"$id\"$class$title><input type=\"checkbox\" name=\"$name\" value=\"$value\"$inputClass id=\"$id\"$selectedHtml />"
	...
	public static function listItem(array $controlOptions, array $popups)
	{
		...
		$output = '
<li class="listItem primaryContent' . $extraClasses . '" id="listItem_' . $controlOptions['id'] . '">
	' . $delete . '
