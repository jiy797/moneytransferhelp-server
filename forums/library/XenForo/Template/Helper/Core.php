	...
	*/
	...
	public static function helperUserBlurb(array $user, $includeUserTitle = true)
	{
		...
		if (!empty($user['location']))
		{
			...
			$location = '<a href="http://maps.google.com' . $mapLink . '" class="concealed" target="_blank" rel="nofollow">' . htmlspecialchars($user['location']) . '</a>';
	...
	public static function convertUrlToActionAndNamedParams($url)
	{
		...
		if (($questPos = strpos($url, '?')) !== false)
		{
			...
			if ($queryString)
			{
				parse_str($queryString, $paramAppend);
				$params = array_merge($params, $paramAppend);
			}
		}
	...
	public static function avatarHtml(array $user, $img, array $attributes = array(), $content = '')
	{
		...
		$src = htmlspecialchars(self::getAvatarUrl($user, $size));
	...
	public static function helperAvatarUrl($user, $size, $forceType = null, $canonical = false)
	{
		...
		$root = ($canonical ? XenForo_Application::get('options')->boardUrl . '/' : '');

		return htmlspecialchars($root . self::getAvatarUrl($user, $size, $forceType));
	}
