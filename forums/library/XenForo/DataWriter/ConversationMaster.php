...
{
	/**
	 * Constant for extra data that holds information about the user that is inviting
	 * additional users to the conversation.
	 *
	 * @var string
	 */
	const DATA_INVITE_USER = 'inviteUser';
	...
	*/
	...
	{
		$permUser = $this->getExtraData(self::DATA_INVITE_USER);
	...
	protected function _postSave()
	{
		...
		{
			$conversationModel->addConversationReplyToRecipients($conversation);
		}
		...
		if ($this->_newRecipients && $recipients = $this->_getUserModel()->getUsersByIds($this->_newRecipients, array('join' => XenForo_Model_User::FETCH_USER_OPTION)))
		{
			...
			$inviteUser = $this->getExtraData(self::DATA_INVITE_USER);

			foreach ($recipients AS $recipient)
			{
				if ($conversationModel->insertConversationRecipient($conversation, $recipient['user_id'], $existingRecipient))
				{
					$conversationModel->insertConversationAlert($conversation, $recipient, $alertAction, $inviteUser);
				}
			}
		}
	}
