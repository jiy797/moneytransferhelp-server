...
class XenForo_Model_SpamCleaner extends XenForo_Model
{
	...
	{
		$mail = new XenForo_Mail('spam_cleaner_applied', array(
			'plainText' => $emailText,
	...
	{
		$ipLogs = $this->_getDb()->fetchAll('
			SELECT * FROM xf_ip
			WHERE ip IN (SELECT ip FROM xf_ip WHERE user_id = ?)
				AND user_id <> ?
				AND user_id <> 0
				AND log_date > ?
			ORDER BY log_date DESC
		', array($userId, $userId, XenForo_Application::$time - $logDays * 86400));
