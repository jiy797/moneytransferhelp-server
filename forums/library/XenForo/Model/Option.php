...
class XenForo_Model_Option extends XenForo_Model
{
	 ...
	 *
	 * @param boolean|null $includeDebug If true, includes debug-only option groups. If null, current debug value is detected.
	 *
	 * @return array Format: [] => group info
	 */
	public function getOptionGroupList($includeDebug = null)
	{
		if ($includeDebug === null)
		{
			$includeDebug = XenForo_Application::debugMode();
		}

		return $this->fetchAllKeyed('
			SELECT *
			FROM xf_option_group
			' . (!$includeDebug ? 'WHERE debug_only = 0' : '') . '
	 ...
	 */
	public function getOptionGroupsByIds(array $groupIds)
	{
		if (!$groupIds)
		{
			return array();
		...
		return $this->fetchAllKeyed('
			SELECT *
			FROM xf_option_group
			WHERE group_id IN (' . $this->_getDb()->quote($groupIds) . ')
		', 'group_id');
	}
	 ...
	 * @return array
	 */
	public function prepareOptionGroups(array $groups)
	{
	 ...
	 * @return array Format: [group id] => info
	 */
	public function getOptionGroupsByAddOn($addOnId)
	{
		return $this->fetchAllKeyed('
			SELECT *
			FROM xf_option_group
	 ...
	 */
	public function getOptionsInGroup($groupId)
	{
		...
		return $this->fetchAllKeyed('
			...
			INNER JOIN xf_option AS f_option ON
				(relation.option_id = f_option.option_id)
	 ...
	 */
	public function getOptionsByIds(array $optionIds)
	{
		if (empty($optionIds))
		{
			return array();
		...
		return $this->fetchAllKeyed('
			SELECT *
			FROM xf_option
			WHERE option_id IN (' . $this->_getDb()->quote($optionIds) . ')
		', 'option_id');
	}
	 ...
	 */
	public function getOptionsByAddOn($addOnId)
	{
		return $this->fetchAllKeyed('
			SELECT *
			FROM xf_option
	...
	public function prepareOptions(array $options)
	{
		...
		{
			$output[$key] = $this->prepareOption($option);
		}
	...
	public function appendOptionsAddOnXml(DOMElement $rootNode, $addOnId)
	{
		...
		foreach ($options AS $option)
		{
			...
			$optionNode->appendChild($document->createElement('default_value', str_replace("\r\n", "\n", $option['default_value'])));
			$optionNode->appendChild($document->createElement('edit_format_params', str_replace("\r\n", "\n", $option['edit_format_params'])));
			$optionNode->appendChild($document->createElement('sub_options', str_replace("\r\n", "\n", $option['sub_options'])));
