...
class XenForo_Model_UserConfirmation extends XenForo_Model
{
	...
	public function sendEmailConfirmation(array $user, array $confirmation = null)
	{
		...
		$mail = new XenForo_Mail('user_email_confirmation', $params, $user['language_id']);
	...
	public function processUserModeration(array $user, $action, $notify = true, $rejectionReason = '')
	{
		...
		if ($action == 'approve')
		{
			...
			if ($notify)
			{
				...
				$mail = new XenForo_Mail('user_account_approved', $params, $user['language_id']);
		...
		else if ($action == 'reject')
		{
			...
			if ($notify)
			{
				...
				$mail = new XenForo_Mail('user_account_rejected', $params, $user['language_id']);
				$mail->send($user['email'], $user['username']);
			}
	...
	public function sendPasswordResetRequest(array $user, array $confirmation = null)
	{
		...
		$mail = new XenForo_Mail('user_lost_password', $params, $user['language_id']);
	...
	public function resetPassword($userId, $sendEmail = true)
	{
		...
		if ($sendEmail)
		{
			...
			$mail = new XenForo_Mail('user_lost_password_reset', $params, $user['language_id']);
			$mail->send($user['email'], $user['username']);
		}
