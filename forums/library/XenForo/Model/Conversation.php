...
class XenForo_Model_Conversation extends XenForo_Model
{
	 ...
	 */
	public function addConversationReplyToRecipients(array $conversation)
	{
		$db = $this->_getDb();
		...
		foreach ($recipients AS $recipient)
		{
			switch ($recipient['recipient_state'])
			{
				case 'active':
					...
					$this->insertConversationAlert($conversation, $recipient, 'reply', null, $extraData);
					break;
	...
	public function insertConversationAlert(array $conversation, array $alertUser, $action,
		array $triggerUser = null, array $extraData = null
	)
	{
		if (!$triggerUser)
		{
		...
		{
			$mail = new XenForo_Mail("conversation_{$action}", array(
				'receiver' => $alertUser,
				'sender' => $triggerUser,
				'options' => XenForo_Application::get('options'),
