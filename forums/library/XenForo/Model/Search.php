...
class XenForo_Model_Search extends XenForo_Model
{
	 ...
	 *
	 * @param array $results List of results, in format [] => array(content_type => x, content_id = y)
	...
	)
	{
		...
		$search = array(
			'search_results' => serialize($results),
			'result_count' => count($results),
			'search_type' => $searchType,
			'search_query' => utf8_substr($searchQuery, 0, 200),
			'search_constraints' => serialize($constraints),
			'search_order' => $order,
			'search_grouping' => $groupByDiscussion ? 1 : 0,
			'warnings' => serialize(array_map('strval', $warnings)),
			'user_id' => $userId,
	...
	{
		$search['searchConstraints'] = unserialize($search['search_constraints']);
		$search['searchWarnings'] = unserialize($search['warnings']);
	 ...
	 *
	 * @param array $results Format: [] => array(content_type => x, content_id => y)
	 *
	 * @return array Format: [content type][] => content id
	 */
	public function groupSearchResultsByType(array $results)
	{
		...
		{
			$resultsGrouped[$result['content_type']][$result['content_id']] = $result['content_id'];
		}
	 ...
	 *
	 * @param array $results Search results ([] => array(content_type => x, content_id => y)
	 * @param array|null $viewingUser Information about the viewing user (keys: user_id, permission_combination_id, permissions) or null for visitor
	 *
	...
	public function getViewableSearchResults(array $results, array $viewingUser = null)
	{
		...
		{
			if (!isset($dataGrouped[$result['content_type']][$result['content_id']]))
			{
	...
	public function getSearchResultsForDisplay(array $results, array $viewingUser = null)
	{
		...
		foreach ($results AS $resultId => $result)
		{
			if (isset($dataGrouped[$result['content_type']][$result['content_id']]))
			{
				$results[$resultId]['content'] = $dataGrouped[$result['content_type']][$result['content_id']];
			}
			else
			{
				unset($results[$resultId]);
	...
	public function sliceSearchResultsToPage(array $search, $page, $perPage)
	{
		...
		{
			$search['searchResults'] = unserialize($search['search_results']);
		}
