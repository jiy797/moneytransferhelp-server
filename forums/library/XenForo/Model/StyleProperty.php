...
class XenForo_Model_StyleProperty extends XenForo_Model
{
	...
	/**
	 * Returns an array of all master style property definitions in the specified group
	 *
	 * @param string $groupId
	 *
	 * @return array
	 */
	public function getMasterStylePropertyDefinitionsByGroup($groupName)
	{
		$properties = $this->fetchAllKeyed('
			SELECT *
			FROM xf_style_property_definition
			WHERE group_name = ?
				AND definition_style_id = 0
			ORDER BY display_order
	...
	public function writeStylePropertyDevelopmentFile(array $definition, array $property)
	{
		...
		if ($includeDefinition)
		{
			...
			$node->appendChild($document->createElement('title',
				isset($definition['masterTitle']) ? $definition['masterTitle'] : strval($definition['title'])
			));

			$node->appendChild($document->createElement('description',
				isset($definition['masterDescription']) ? $definition['masterDescription'] : strval($definition['description'])
			));
		}
		else
		{
	...
	public function appendStylePropertyXml(DOMElement $rootNode, $styleId, $addOnId = null)
	{
		...
		foreach ($groups AS $group)
		{
			if ($addOnId === null || $group['addon_id'] == $addOnId)
			{
				...
				$node->appendChild($document->createElement('title', $group['title']));
				$node->appendChild($document->createElement('description', $group['description']));
			}
		}
	}
