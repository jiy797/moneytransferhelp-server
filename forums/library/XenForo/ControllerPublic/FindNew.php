...
class XenForo_ControllerPublic_FindNew extends XenForo_ControllerPublic_Abstract
{
	...
	public function actionThreads()
	{
		...
		if (!$search || $search['search_type'] != 'new-threads' || $search['user_id'] != XenForo_Visitor::getUserId())
		{
			return $this->findNewThreads();
		}
		...
		foreach ($results['results'] AS $result)
		{
			...
			$threads[$result['content_id']] = $thread;
		}
	...
	public function findNewThreads()
	{
		...
		$search = $searchModel->insertSearch($results, 'new-threads', '', array(), 'date', false);
