...
class XenForo_ViewRenderer_Xml extends XenForo_ViewRenderer_Abstract
{
	...
	public function renderRedirect($redirectType, $redirectTarget, $redirectMessage = null, array $redirectParams = array())
	{
		...
		$rootNode->appendChild($document->createElement('_redirectStatus', 'ok'));
		$rootNode->appendChild($document->createElement('_redirectTarget', $redirectTarget));
		$rootNode->appendChild($document->createElement('_redirectMessage',
			(is_null($redirectMessage) ? new XenForo_Phrase('redirect_changes_saved_successfully') : $redirectMessage)
		));
		$rootNode->appendChild($document->createElement('jsonParams', XenForo_ViewRenderer_Json::jsonEncodeForOutput($redirectParams)));
