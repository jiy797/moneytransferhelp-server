 ...
 */
class XenForo_ControllerAdmin_Template extends XenForo_ControllerAdmin_Abstract
{
	...
	public function actionSave()
	{
		...
		else
		{
			...
			return $this->responseRedirect(
				...
				XenForo_Link::buildAdminLink('styles/templates', $style, array('last' => $writer->get('template_id')))
			);
		}
	}
	...
	public function actionSaveMultiple()
	{
		...
		else
		{
			...
			if ($templateId && $last = $templateModel->getTemplateById($templateId))
			{
				...
				$extraData['last'] = $last['template_id'];
			}

			return $this->responseRedirect(
				XenForo_ControllerResponse_Redirect::SUCCESS,
				XenForo_Link::buildAdminLink('styles/templates', $style, $extraData)
			);
		}
	}
